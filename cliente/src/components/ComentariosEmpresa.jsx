...
export const ComentariosEmpresa = ({ id_empresa }) => {
  ...
  const fetchComentarios = async () => {
    try {
      const { data } = await axios.get(`${backendUrl}/api/comentario/empresa/${id_empresa}`);
  ...
  const fetchPromedio = async () => {
    try {
      const { data } = await axios.get(`${backendUrl}/api/comentario/promedio_emp/${id_empresa}`);
      if (data?.success && data.promedio !== undefined) {
        setPromedio(parseFloat(data.promedio) || 0);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const enviarComentario = async (e) => {
    ...
    try {
      await axios.post(`${backendUrl}/api/comentario/comentar_emp`, {
        id_empresa,
        contenido,
        calificacion,
      });
